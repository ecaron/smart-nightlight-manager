...
if (!process.env.DISABLE_BUTTON) {
  ...
  dash.on('detected', function (dash_id) {
    logger.info('Got click for ' + dash_id);
    var button = db('buttons').find({mac: dash_id});
    if (typeof button !== 'object' || !button.light) return;
    if (ignorePushes[dash_id]) {
      light.addLog(sprintf('Ignoring duplicate button push for light %d', lightId));
      return;
    }
    ...
    exports.api.lightStatus(lightId, function (err, result) {
      ...
      ignorePushes[dash_id] = true;
      setTimeout(function () {
        ignorePushes[dash_id] = false;
      }, 30 * 1000);
      if (result.state.on) {
        light.turnOff();
      } else {
