...
exports.getState = async function (light) {
  let rawState
  try {
    rawState = await axios.get(`http://${light.settings.ip}/all`)
  } catch (e) {
    return false
  }
  const state = {}
...
exports.turnOn = async function (light, lightSettings) {
  if (!lightSettings.pattern) lightSettings.pattern = 'Solid Color'
  if (!lightSettings.palette) lightSettings.palette = 'Heat'
  if (lightSettings.brightness) {
    await axios.post(`http://${light.settings.ip}/brightness?value=${Math.round(lightSettings.brightness * 2.55)}`)
  }
  ...
  if (lightSettings.color && lightSettings.pattern === 'Solid Color') {
    const hex = hex2rgb(lightSettings.color)
