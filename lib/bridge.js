...
exports.api.lights(function(err, data) {
  exports.lights[data.id] = {
    ...
    log: [],
    turnOn: function(){
      this.log(sprintf('Turning on light %d.', this.id));
    ...
    turnOff: function(skipConsole){
      ...
      exports.api.setLightState(this.id, {on: false}, function(err, result) {
        ...
        if (err) throw err;
        if (!skipConsole) {
          self.log(sprintf('Turning off light %d.', self.id));
        }
      });
    },
    turnOnWithTimer: function(){
      ...
      self.timer = setTimeout(function(){
        self.turnOff(true);
        self.log(sprintf('%d minutes reached, turning off light %d.', stayOnMinutes, self.id));
